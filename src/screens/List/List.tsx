...
import { Container, Main, ContinentList, NoFoundText, ShowAllButton, ShowAllText, Title, Table } from './list';

interface IList {
  navigation: MainStack;
}

export default function List({navigation}: IList) {
  ...
  const showLandmarksByContinent = (name:string) => {
    if(name) {
      const filtered = landmarks.filter(landmark => landmark.continent === name);
  ...
  const renderLandmarks = () => {
    if(filteredLandmarks.length) {
      return filteredLandmarks.map(landmark => (
        <SharedElement id={`item.${landmark.id}.icon`}>
          <LandmarkCard key={landmark.id} {...{landmark, onPressLandmark}}/>
        </SharedElement>
      ));
    }
    return (
      <NoFoundText>No landmark found...</NoFoundText>
    )
  }
    ...
    <Container>
      ...
      <Main>
        ...
        <FlatList
          ...
          renderItem={({item})=> (
            <ContinentCard continent={item} {...{showLandmarksByContinent}}/>
          )}
        />
        </ContinentList>
